 ...
 * @date 2018/5/3
 */
public class RequestRibbonForwardUtils {

    private static final Logger LOGGER = LoggerFactory.getLogger(RequestRibbonForwardUtils.class);

    private static final String ENCODING_UTF8 = "UTF-8";

    public static RibbonCommandContext buildCommandContext(final HttpServletRequest req,
                                                           final List<RibbonRequestCustomizer> requestCustomizers,
        ...
        contentLength = 0L;
        return new RibbonCommandContext(service, verb, uri, false, headers, params,
                requestEntity, requestCustomizers, contentLength);
    }

    public static String buildZuulRequestUri(HttpServletRequest request) {
    ...
    public static ClientHttpResponse forward(final RibbonCommandContext context,
                                             final RibbonCommandFactory<?> ribbonCommandFactory) throws ZuulException {
    ...
    public static String getHelperServiceByUri(final HelperZuulRoutesProperties helperZuulRoutesProperties, final String uri) {
        try {
            String[] uris = uri.split("/");
            if (uris.length <= 1) {
                return null;
            }
            String realUri = uris[1];
            ZuulRoute route = helperZuulRoutesProperties.getRoutes().get(realUri);
            if (route == null) {
                return null;
            }
            String helperService = route.getHelperService();
            if (StringUtils.isEmpty(helperService)) {
                return null;
            }
            return helperService;
        } catch (Exception e) {
            LOGGER.info("getHelperServiceByUri error, {}", e.getMessage());
            return null;
        }
    }
